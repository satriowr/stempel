            ...
            <div class="table-responsive">
              <table class="table table-bordered table-striped" id="table-datatable">
                ...
                <tbody>
                  <?php
                  $no = 1;
                  $invoice = mysqli_query($koneksi, "SELECT * from invoice,customer,transaksi where customer_id=invoice_customer AND invoice_transaksi=transaksi_id order by invoice_id desc");
                  while ($i = mysqli_fetch_array($invoice)) {
                    $id_invoice = $i['invoice_id'];
                    $nomor_resi = $i['invoice_resi'];
                  ?>
                    <tr>
                      <td><?php echo $no++; ?></td>
                      ...
                      <td><?php echo $i['transaksi_catatan'] ?></td>
                      ...
                      <td class="text-center">
                        ...
                        if ($i['invoice_status'] == 0) {
                          ...
                          if ($i['invoice_resi'] == '') {
                            ...
                            echo "<form method='post' action='inputresi.php'>
  <input type='hidden' type='text' value='$id_invoice' name='id_invoice'>
  <input type='text' class='form-control' style='margin-top:10px' name='resi' placeholder='Masukan nomor Resi'>
    <button type='submit' style='margin-top:10px' class='btn btn-primary btn-sm'><i class='fa fa-plus-circle' aria-hidden='true'></i> Input</button>
</form>";
                          } else {
                            echo "<span class='label label-primary'>Dikirim - Nomor Resi : $nomor_resi</span>";
                          }
                          ...
                          <form action="transaksi_status.php" method="post">
                            ...
                            <select name="status" id="" class="form-control" onchange="form.submit()">
                              ...
                              <!-- <option <?php if ($i['invoice_status'] == "2") {
                                              echo "selected='selected'";
                                            } ?> value="2">Dikonfirmasi</option> -->
                              <option <?php if ($i['invoice_status'] == "3") {
                                        echo "selected='selected'";
                                      } ?> value="3">Dikonfirmasi Dan Diproses</option>
                              <option <?php if ($i['invoice_status'] == "4") {
                                        echo "selected='selected'";
                                      } ?> value="4">Dikirim</option>
                              <!-- <option <?php if ($i['invoice_status'] == "5") {
                                              echo "selected='selected'";
                                            } ?> value="5">Pesanan Sampai</option> -->
                              <option <?php if ($i['invoice_status'] == "6") {
                                        echo "selected='selected'";
                                      } ?> value="6">Batalkan</option>
                            </select>
                          </form>
                        <?php } ?>
                      ...
                      <td class="text-center">
                        <?php if ($i['invoice_status'] == "5") { ?>
                          <a class='btn btn-sm btn-success' href="rincian_pesanan.php?id=<?php echo $i['invoice_id']; ?>"><i class="fa fa-print"></i> Invoice</a>
                        <?php } else { ?>
